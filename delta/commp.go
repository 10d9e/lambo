// Package deltacommp allows calculating a Filecoin Unsealed Commitment (commP/commD)
...
func (cp *Calc) Prepend(input []byte) (int, error) {
	totalInputBytes := len(input)
	if len(input) > 0 {
		cp.buffer = append(input, cp.buffer...)
	}
	return totalInputBytes, nil
}
